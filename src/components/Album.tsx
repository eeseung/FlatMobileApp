      ...
      {state === 'PLAY_SMALL' ?
            ...
            <PlayButton >
              <TouchableOpacity onPress={() => setPlayed(!played)}>
                {played ?
                  ...
                  :
                  <Image source={require('./assets/images/Album_Play.png')} style={{width: 15, height: 18}}/>
                }
              </TouchableOpacity>
            </PlayButton>
          </AlbumCover>
        ...
        <AlbumContainer size={ALBUM_SIZE[state]}>
            ...
            <PlayButton >
              <TouchableOpacity onPress={() => setPlayed(!played)}>
                {played ?
                  ...
                  :
                  <Image source={require('./assets/images/Album_Play.png')} style={{width: 60, height: 72}}/>
                }
          ...
          <AlbumText isHorizontal={false}>
            {state === 'LIKE_SMALL' && <Text style={{ fontSize: 12 }}>{title}</Text> }
            {state === 'LIKE_LARGE' &&
            <><Text style={{ fontWeight: "bold", fontSize: 16 }}>{title}</Text>
              <Text style={{ fontSize: 12 }}>{artist} {time}</Text></>
            }
